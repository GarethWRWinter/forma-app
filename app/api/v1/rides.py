...
def list_rides(
    ...
    # Never re-title a ride the rider named themselves.
    missing = [
        r for r in rides
        if not r.title_is_custom and (not r.story or not r.forma_title)
    ][:8]
    if missing:
...
class RideUpdate(BaseModel):
    """What a rider may correct on their own ride."""
...
def update_ride(
    ...
    """Rename a ride, or correct where it happened.

    Titles are generated by the classifier and then by Forma, which is fine
    for a Tuesday turbo and useless for the ride that matters: a hill climb
    PB arrives called "Sprint Training". A name the rider chose outranks
    both, and title_is_custom stops the re-titler undoing it.
    """
    ride = (
        db.query(Ride)
    ...
    if not data:
        raise BadRequestException(detail="Nothing to update")

    if "title" in data and data["title"]:
        title = data["title"].strip()
        if not title:
            raise BadRequestException(detail="A ride needs a name")
        ride.title = title
        # forma_title is what the list renders, so it has to move too or the
        # rider's name would be invisible behind the AI's.
        ride.forma_title = title
        ride.title_is_custom = True

    if "location_name" in data:
        ride.location_name = (data["location_name"] or "").strip() or None
